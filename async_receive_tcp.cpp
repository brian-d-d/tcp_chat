...
class tcp_client {
    public: 
        ...
        : _io_context(io_context), _socket(io_context), _stdin(io_context, ::dup(STDIN_FILENO)) {
            ...
            read_from_socket();
            // write_to_host("hello making sure\n");
            read_from_stdin();
        }
        ...
        void read_from_stdin() {
            _stdin.async_read_some(boost::asio::buffer(_stdin_buffer, 15), 
    ...
    private:
        ...
        // boost::asio::streambuf _stdin_buffer;
        ...
        void handle_read_stdin(const boost::system::error_code& error, std::size_t bytes_transferred) {
            ...
            // _stdin.io_control(command);
            // std::size_t bytes_readable = command.get();
            // std::cout << "bytes ready: " << bytes_readable << std::endl;
            // std::cout << "bytes: " << bytes_transferred << std::endl;
            // for (std::size_t i = 0; i < bytes_transferred; i++) {
            //     std::cout << _stdin_buffer[i];
            // }
